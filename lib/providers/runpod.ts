...
export async function generateImageWithRunpod(params: ImageGenParams) {
  ...
  const apiKey = params.apiKey || process.env.RUNPOD_API_KEY
  if (!apiKey) {
    throw new Error("RUNPOD_API_KEY is not configured. Set it in .env.local")
  }
  console.log("[runpod] API key present:", apiKey ? `${apiKey.slice(0, 8)}...` : "MISSING")
  ...
  let sanitizedProviderOptions: any | undefined
  {
    ...
    const mime = first && first.startsWith("data:") ? first.slice(5, first.indexOf(";")) : undefined
  ...
  try {
    ...
    console.error("Model:", meta.id)
    console.error("Prompt:", requestPayload.prompt)
    console.error("Error:", err.message)
    if (err.cause) {
      console.error("Cause:", JSON.stringify(err.cause, null, 2))
    }
    console.error("Provider options:", sanitizedProviderOptions)
    console.error("======================================\n")
    throw err
  }
