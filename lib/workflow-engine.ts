...
export class WorkflowEngine {
  ...
  private async executeImageGenNode(node: WorkflowNode) {
    ...
    if (wfId) {
      ...
      if (imageCandidates.length > 0) {
        // Collect ALL connected images (sort by timestamp, newest first)
        const sortedCandidates = imageCandidates.slice().sort((a: any, b: any) => {
          const at = a.result?.metadata?.timestamp
            ? new Date(a.result.metadata.timestamp).getTime()
            : 0
          const bt = b.result?.metadata?.timestamp
        ...
        // Load all images from all candidates
        for (const candidate of sortedCandidates) {
          const ref: AssetRef | undefined = candidate?.result?.assetRef
          ...
          if (!url) url = candidate?.result?.data
          if (url) inputImageUrls.push(url)
        }

        // Keep first image in inputImageUrl for backward compatibility
        inputImageUrl = inputImageUrls[0]
        console.log(
          `[WorkflowEngine] Collected ${inputImageUrls.length} image(s) from connected nodes`
        )
      }
    ...
    const response = await fetch("/api/generate-image", {
      ...
      body: JSON.stringify({
        ...
        inputs: hasImageInput ? { images: inputImageUrls } : undefined,
      }),
    })
    ...
    console.log(`[WorkflowEngine] Image generation API response received for node ${node.id}:`, {
    ...
    if (!result.success) {
      throw new Error(result.error || result.details || "Image generation failed")
    }

    console.log(`[WorkflowEngine] Saving asset to AssetManager for node ${node.id}`)
    ...
    console.log(
      `[WorkflowEngine] Asset saved successfully for node ${node.id}, assetRef:`,
      assetRef
    )
    ...
    try {
      const wfId = (node as any).workflowId as string | undefined
      if (wfId) {
        console.log(`[WorkflowEngine] Updating node result in store for node ${node.id}`)
        const { updateNodeResult } = useWorkflowStore.getState()
        updateNodeResult(wfId, node.id, node.result, [newResult])
        console.log(`[WorkflowEngine] Node result updated successfully for node ${node.id}`)
      }
    } catch (err) {
      console.error(`[WorkflowEngine] Failed to update node result for node ${node.id}:`, err)
    }
  }
  ...
  private async executeImageEditNode(node: WorkflowNode) {
    ...
    const result = await response.json()
    console.log(`[WorkflowEngine] Image edit API response received for node ${node.id}:`, {
      success: result.success,
      hasImageUrl: !!result.imageUrl,
      executionId: result.executionId,
    })
    ...
    if (!result.success) {
      throw new Error(result.error || result.details || "Image edit failed")
    }

    console.log(`[WorkflowEngine] Saving edited asset to AssetManager for node ${node.id}`)
    ...
    console.log(
      `[WorkflowEngine] Edited asset saved successfully for node ${node.id}, assetRef:`,
      assetRef
    )
    ...
    try {
      ...
      if (wfId) {
        console.log(
          `[WorkflowEngine] Updating node result in store for node ${node.id} (image edit)`
        )
        const { updateNodeResult } = useWorkflowStore.getState()
        updateNodeResult(wfId, node.id, node.result, [newResult])
        console.log(
          `[WorkflowEngine] Node result updated successfully for node ${node.id} (image edit)`
        )
      }
    } catch (err) {
      console.error(`[WorkflowEngine] Failed to update node result for node ${node.id}:`, err)
