...
import { MessageSquare, ImageIcon, ImagePlus, X, Video, Wand2, Settings2, Plus, ChevronDown, Play } from "lucide-react"
        ...
        <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
          <CollapsibleContent className="space-y-2 mt-2">
            ...
            <div className="pt-2 border-t border-border/30">
              ...
              >
                <ChevronDown className={`w-3 h-3 mr-2 transition-transform ${showMeta ? "rotate-180" : ""}`} />
                Debug Info
              </Button>
              {showMeta && (
...
const ImageGenNode = ({ data, id }: { data: any; id: string }) => {
  ...
  const [isExpanded, setIsExpanded] = useState(false)
  const [showMeta, setShowMeta] = useState(false)
  const imageUrl: string | undefined = data?.result?.type === "image" ? data?.result?.data : undefined
  const isRunning = data.status === "running"
  const [localImage, setLocalImage] = useState<string | undefined>(data.config?.localImage || undefined)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        } else if (data.config?.localImage && !data.config?.localImageRef && typeof indexedDB !== "undefined") {
        ...
        <div className="space-y-2">
          {imageUrl && (
            <div className="relative overflow-hidden rounded border group">
              <img src={imageUrl || "/placeholder.svg"} alt="Node output" className="w-full h-auto object-contain" />
              <Button
                variant="ghost"
                size="sm"
          ...
          {!imageUrl && localImage && (
            <div className="relative overflow-hidden rounded border group">
              <img src={localImage || "/placeholder.svg"} alt="Local image" className="w-full h-auto object-contain" />
              <Button
                variant="ghost"
                size="sm"
                className="absolute top-2 right-2 h-6 w-6 p-0 bg-background/80 hover:bg-background opacity-0 group-hover:opacity-100 transition-opacity"
          ...
          <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
            <CollapsibleContent className="space-y-2 mt-2">
              <div>
                <Label className="text-xs text-muted-foreground mb-1 block">Load local image</Label>
                <Input
                  type="file"
                  accept="image/*"
              ...
              <div className="grid grid-cols-1 gap-3 mt-4">
                {nodeTypeConfig.map((nodeType) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`w-5 h-5 ${getIconColor(nodeType.id)} mt-0.5 flex-shrink-0`} />
