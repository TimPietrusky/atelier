...
export default function SignInClient({ returnPathname }: { returnPathname?: string }) {
  const [loading, setLoading] = useState(false)

  const handleGoogleSignIn = async () => {
    setLoading(true)
    // Use returnPathname prop from server component (Next.js pattern)
    const returnPath = returnPathname || "/workflow"
    // Redirect to API route that initiates Google OAuth directly
    // This bypasses WorkOS hosted UI and goes straight to Google
    window.location.href = `/api/auth/google?return_pathname=${encodeURIComponent(returnPath)}`
  }

  return (
    <div className="h-full w-full md:h-[400px] md:max-w-4xl md:w-full text-center flex flex-col">
      {/* Fixed-size centered box - same on both pages, content aligned to top */}
